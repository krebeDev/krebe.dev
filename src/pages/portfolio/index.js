...
const Projects = ({ data }) => {
	...
	useEffect(() => {
		...
		setProjects((cs) => filter === ALL_PROJECTS ? data.allMdx.nodes : filtered)
		setIsFiltering(true)
	...
	return (
		<Layout>
			...
			<h1>Frontend Developer Portfolio</h1>
				<p>A collection of my featured projects. You can find more of my work on <a 
				rel='noopenner noreferre'	href={data.site.siteMetadata.author.socialProfiles.github}
				className={styles.githubLink}>Github</a>.</p>
			...
			{!isFiltering && 	<p>{filter === ALL_PROJECTS 
					? `Showing ${ALL_PROJECTS.toUpperCase()}: Apply the above filters to list them by tool or technology.` 
					: `Showing ${projects.length} Project${isSingle ? '' : 's'} filtered by ${filter.toUpperCase()}` }</p>}
			<div className={styles.projectList}>
				<ProjectCards projects={projects} />
				{isFiltering && <div className={styles.filtering}>
					<p className={styles.filterStatus}>Filtering projects by: {filter}... </p>
				</div>}
			</div>
			</div>	
			</section>
