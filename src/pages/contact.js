...
import { faUserAlt, faEnvelope, faEdit } from '@fortawesome/free-solid-svg-icons'
			...
			<h1>Get in touch</h1>
				...
				<div className={styles.socials}>
					...
					<ul className={styles.profileLinks}>
						{Object.entries(siteMetadata.author.socialProfiles).map(([key, value]) => <li key={key} className={styles.profile}>
							<a href={value} target='_blank' rel='noopener noreferrer' className={`${styles.profileLink} ${styles[key]}`}>
							<span>
							{socialIcons[key]}					
							</span>
							<span className={styles.networkName}>{toTitleCase(key)}</span>
							</a>
						</li>)} 
					</ul>
				</div>
